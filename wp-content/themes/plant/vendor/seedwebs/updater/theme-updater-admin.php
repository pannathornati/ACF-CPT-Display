...
class EDD_Theme_Updater_Admin {
	...
	function license_menu() {
		...
		// Show menu only on parent theme (Plant, not Fruit).
                    ...
                    <td>
                        <?php 
							if ( $license && ('valid' == $status) ) { 
								$license_output = substr($license, 0, 4) . "-••••-••••-••••-" . substr($license, -4, 4);
							} else {
								$license_output = $license;
							}
						?>

                        <input id="<?php echo $this->theme_slug; ?>_license_key" name="<?php echo $this->theme_slug; ?>_license_key" type="text" class="regular-text" value="<?php echo esc_attr( $license_output ); ?>" />
